...
void nactiSoubor(Tseznam* sopky, FILE *f){
    char aktivni[3], jmeno[100];
    int vyska;
    while(fscanf(f, "%99s;%d;%s", jmeno, &vyska, aktivni) == 3){
...
float prumerVysek(Tseznam *s, bool aktivni){
    ...
    for(int i=0; i<s->delka; i++){
        if(s->sopky[i]->aktivni)
            soucet += s->sopky[i]->vyska;
    }
    return soucet / (s->delka * 1.0);
}
...
void vypisSopkuSoubor(Tsopka *s, FILE *f){
    if(s->aktivni) fprintf(f ,"%s;%d;ANO", s->nazev, s->vyska);
    else fprintf(f, "%s;%d;NE", s->nazev, s->vyska);
}
